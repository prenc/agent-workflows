...
from github_workflows.runtime import GIT_TIMEOUT_SECONDS, WorkflowRuntime
...
class TestRuntimeSafety:
    @staticmethod
    def curation_assignment(runtime: WorkflowRuntime, issue: int) -> dict[str, Any]:
            ...
            linked = runtime.task_manage(
                TaskManageRequest(
                    ...
                    task={
                        ...
                        "assignment": {
                            "mode": "verify",
                            "candidate": {"id": "another-candidate"},
                            "validation_ids": ["probe-output"],
                        },
                    },
                )
            )
            explicit = runtime.task_context(linked["task_ref"])["validation"]
            assert explicit["record_count"] == 1
            assert explicit["records"][0]["id"] == "probe-output"
        ...
        with tempfile.TemporaryDirectory(prefix="runtime-execution-blocked-") as directory:
            ...
            runtime.task_manage(
                TaskManageRequest(
                    ...
                    task={
                        "logical_id": "unit-b",
                        "assignment": self.curation_assignment(runtime, 2),
                    },
                )
            )
    ...
    def test_history_query_is_bounded_and_requires_a_selector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="history-selector-") as directory:
            ...
            assert runtime.state(workflow)["history"]["default_sha"] == "a" * 40
            runtime.history_manage(HistoryManageRequest(action="prepare", workflow=workflow))
            inherited = runtime.history_manage(
                HistoryManageRequest(action="commit", workflow=workflow)
            )
            assert inherited["history"]["default_sha"] == "a" * 40
            ...
            fresh_root.mkdir()
            fresh = self.make_runtime(fresh_root)
            fresh.run_manage(
                RunManageRequest(action="start", workflow=workflow, repository="example/other")
            )
            fresh.history_manage(HistoryManageRequest(action="prepare", workflow=workflow))
            with pytest.raises(ValueError, match="requires default_sha"):
                fresh.history_manage(HistoryManageRequest(action="commit", workflow=workflow))
                    ...
                    TaskManageRequest(
                        ...
                        task={
                            ...
                            "assignment": self.curation_assignment(
                                second, 1 if task_id.startswith("running") else 2
                            ),
                        },
                    )
                )
                ...
                planned = runtime.task_manage(
                    TaskManageRequest(
                        ...
                        task={
                            ...
                            "assignment": self.curation_assignment(
                                runtime, 1 if logical_id.endswith("one") else 2
                            ),
                        },
                    )
                )
    ...
    def test_suspended_generic_run_accepts_only_late_worker_results(self) -> None:
        with tempfile.TemporaryDirectory(prefix="runtime-generic-suspend-") as directory:
            runtime = self.make_runtime(Path(directory))
            workflow = "gh-implement-issue"
            runtime.run_manage(
                RunManageRequest(
            ...
            for request in (
                TaskManageRequest(action="plan", workflow=workflow, task={"logical_id": "late"}),
                TaskManageRequest(
                    action="integration_begin",
                    workflow=workflow,
                    task_id=planned["task_id"],
    ...
    def test_audit_task_context_reports_an_exact_history_window(self) -> None:
        with tempfile.TemporaryDirectory(prefix="runtime-audit-history-window-") as directory:
            ...
            first_keys = {(item["kind"], item["number"]) for item in selection["records"]}
            assert (continued["records"][0]["kind"], continued["records"][0]["number"]) not in (
                first_keys
            )
            ...
            other = runtime.task_manage(
                TaskManageRequest(
                    ...
                    task={
                        ...
                        "assignment": {"mode": "discover", "area": "area/other"},
                    },
                )
            )
    ...
    @pytest.mark.parametrize(
        ("stalled_call", "message"),
        [
            (1, "check-ignore timed out"),
            (2, "rev-parse timed out"),
            (3, "verification check-ignore timed out"),
        ],
    )
    def test_worktree_root_git_times_out(self, stalled_call, message) -> None:
        with tempfile.TemporaryDirectory(prefix="runtime-check-ignore-") as directory:
            runtime = self.make_runtime(Path(directory))
            real_run = subprocess.run
            calls = 0

            def stalled(cmd, *args, **kwargs):
                nonlocal calls
                calls += 1
                assert kwargs["timeout"] == GIT_TIMEOUT_SECONDS
                if calls == stalled_call:
                    raise subprocess.TimeoutExpired(cmd=cmd, timeout=kwargs["timeout"])
                return real_run(cmd, *args, **kwargs)

            with mock.patch("github_workflows.runtime.subprocess.run", side_effect=stalled):
                with pytest.raises(ValueError, match=message):
                    runtime._worktree_root()
            assert calls == stalled_call
    ...
    def test_discard_stale_run_worktree_removal_times_out(self, monkeypatch, cached) -> None:
        with tempfile.TemporaryDirectory(prefix="runtime-stale-timeout-") as directory:
            runtime = self.make_runtime(Path(directory))
            monkeypatch.setenv("XDG_CACHE_HOME", str(Path(directory) / "xdg-cache"))
            self.initialize_audit(runtime)
            state = runtime.state("gh-audit-repo")
            root = runtime._cache_worktree_path() if cached else runtime.workspace / ".worktrees"
            worktree = root / f"gh-audit-pr-{runtime._run_ref(state['run_id'])}-95-abcdef0"
            worktree.mkdir(parents=True)
            blocked = subprocess.TimeoutExpired(cmd=["git"], timeout=GIT_TIMEOUT_SECONDS)
            with mock.patch("github_workflows.runtime.subprocess.run", side_effect=blocked) as run:
                with pytest.raises(ValueError, match="stale probe worktree removal timed out"):
                    runtime._discard_stale_run("gh-audit-repo")
            assert run.call_args.kwargs["timeout"] == GIT_TIMEOUT_SECONDS
            assert run.call_args.args[0][-4:] == ["worktree", "remove", "--force", str(worktree)]
            assert runtime.state("gh-audit-repo") == state
