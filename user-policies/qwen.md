...
Place Git worktrees under `<project>/.worktrees/<name>` and ensure `.worktrees/` is ignored by the main worktree. In each secondary worktree, link `.venv` to the main project's `.venv`; do not create, copy, or synchronize a separate environment.
